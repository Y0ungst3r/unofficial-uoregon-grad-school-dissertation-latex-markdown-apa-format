...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../Pictures/Chap_2/Particle_zoo.jpg}
    \caption{Standard model elementary particles on the left and their superpartners on the right. \cite{particle_zoo_susy}.}
...
where $\Phi$ is the complex scalar Higgs doublet, $m_H^2$ is the negative mass squared parameter, and $\lambda$ is the Higgs self coupling parameter. Radiative corrections from loop diagrams involving Standard Model particles modify the Higgs mass parameter according to
\[
m_H^2 = m_{H,\mathrm{bare}}^2 + \Delta m_H^2 .
\]
For example, a fermion loop, as illustrated in Fig.~\ref{fig:loop_corrections}, gives a correction of the form
\begin{equation}
...
In addition to the hierarchy problem, another open problem in particle physics is the existence of dark matter. Astrophysical and cosmological observations provide strong evidence for the existence of large amounts of non-luminous matter, known as dark matter, in the universe. One of the earliest pieces of evidence comes from measurements of galactic rotation curves, which show that star's rotational velocities in galaxies at large radii remain approximately constant, as seen in Fig. \ref{fig:loop_corrections}. In contrast, if visible matter accounted for the majority of the galactic mass, the rotational velocity of stars would be expected to decrease with increasing distance from the galactic center. Additional evidence for dark matter arises from observations such as gravitational lensing and the dynamics of galaxy clusters, both of which indicate the presence of additional mass contributing to the gravitational potential.
\begin{figure}[!h]
    \centering
        \includegraphics[width=\textwidth]{../Pictures/Chap_2/Rotation.jpg}
    \caption{Rotation curves for NGC 6503, Note the circle marks with error bars are the measured data. \cite{Rotation_Curves}}
    \label{fig:loop_corrections}
\end{figure}
...
The full set of predicted superpartners is illustrated in Fig.~\ref{fig:particle_zoo}. The scalar superpartners of the Standard Model leptons and quarks are collectively referred to as sleptons and squarks, respectively. Their names are formed by prefixing the corresponding Standard Model particle name with ``s'', for example the scalar partner of the tau lepton is the stau. Similarly, the fermionic superpartners of the Standard Model gauge bosons are known as gauginos. Their names are conventionally formed by appending the suffix ``ino'' to the corresponding gauge boson name.

Supersymmetric particles are conventionally denoted using the same symbol as their Standard Model partners with a tilde placed above the symbol to distinguish them. Following electroweak symmetry breaking, the charged and neutral gauginos mix with the higgsinos; the gauge eigenstates are not identical to the mass eigenstates. The resulting charged mass eigenstates are known as charginos and the neutral mass eigenstates as neutralinos, denoted by \chinopm\ and \nino, respectively. 
...
The scenarios relevant to this dissertation are theoretically motivated and involve long-lived charged particles that decay into Standard Model states. In all cases discussed, some or all of the decay products escape detection, resulting in missing energy signatures. Although these are not the only mechanisms that can produce long-lived charged particles in beyond the Standard Model scenarios, these scenarios provide a well-motivated and experimentally accessible region of parameter space. This makes them a natural and systematic target for the searches presented in this dissertation.
...
In the region where $\Delta m \approx m_\tau$ the width is significantly reduced and the $\stau$ is long-lived, with a typical lifetime of $\mathcal{O}(1~\mathrm{ns})$, which is relevant for the phenomenology considered in this dissertation. 
...
\label{sec:GMSB}
Another scenario of interest for this dissertation is gauge-mediated supersymmetry breaking (GMSB).
...
The final scenario of interest to this dissertation is Anomaly Mediated Supersymmetry Breaking (AMSB). In this scenario, supersymmetry is assumed to be broken in some hidden sector, similarly to GMSB. In AMSB the breaking is communicated to the visible sector through superconformal anomalies in supergravity. 
